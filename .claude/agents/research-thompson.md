...
**重要**：在工作过程中，如果你调用了需要长时间执行的技能工具（如 deep-research、competitive-intelligence-analyst、web 搜索等，预计执行时间 > 2 分钟），请遵循以下进度报告规范：

### 1. 任务开始前
立即发送消息告知 team lead：
```
正在执行 [技能名称] 技能，预计需要 [X] 分钟。
任务内容：[简要说明]
```
...
**目的**：这样可以让 team lead 和用户了解你的工作进度，避免误以为你卡住了或没有在工作。保持透明的沟通是团队协作的关键。
