...
主 agent 应提供：
- **协议规范**：API endpoint、请求/响应格式（来自 protocol-analyst）
- **用户场景**：目标用户、使用场景、痛点
- **技术约束**：依赖库、平台限制、性能要求
- **参考设计**：类似工具的 CLI 设计（如有）

## 工作模式

- **Fresh Agent Per Task**：每个 API 设计任务派发一个新 agent
- **主 agent 提供完整上下文**：不读取协议文档，由主 agent 提取关键信息后传递
- **聚焦设计**：只完成 API 设计，不做实现或文档编写
- **用户视角**：从用户体验出发，而不是技术实现
