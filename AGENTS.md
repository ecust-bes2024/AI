...
这里不是单一项目仓库，而是实验室工作区。默认用途是：

- 研究外部资料、仓库、工作流和方法
- 试验新工具、新 agent、新提示词和新工作方式
- 在进入正式工程前先做隔离评估
- 将有价值的方法吸收到长期资产中

### 目录分工

- `lab/`：原型、快速 demo、试验性实现
- `research/`：研究、分析、吸收和对比
- `learn-temp/`：外部 repo / 工具 / 文章的临时评估区
- `sandbox/`：隔离验证、冒烟测试、高风险试验
- `projects/`：从实验结果中逐步升级出来、开始接近正式工程的项目区
- `docs/`：沉淀结论、设计、方法和工作流文档

### 默认工作方式

在本实验室工作区中，默认按“先评估，再决定是否吸收”的方式工作，而不是默认直接集成或长期维护。

对外部仓库、文章、工具、技能、workflow 的默认顺序是：

1. 先在实验室中隔离评估
2. 明确问题、假设和验证目标
3. 给出结论
4. 再决定是丢弃、记录、吸收，还是升级为正式项目

### 实验产出出口

实验结果不能只停留在对话中，默认应进入以下出口之一：

- `discard`：丢弃
- `keep-as-note`：保留为研究笔记或方法结论
- `absorb-to-memory`：吸收为长期偏好、决策或经验
- `promote-to-project`：升级到正式项目或更稳定的工程资产

### 实验室与正式项目的边界

本工作区默认允许轻量试验、快速验证和低成本探索，但不应把实验区误当成正式项目区。

当任务已经满足以下特征时，应考虑迁移到项目级规则：
...
在这些情况下，应进入具体项目仓库，而不是继续停留在实验室目录中。

## 语言工作流

适用范围：主 agent、子 agent，以及所有基于本工作区 `AGENTS.md` 继承指令的对话。

- 当用户用中文表达任务时，默认先在内部将任务重写为更清晰、更完整、更适合模型执行的英文任务描述，再基于该英文任务进行推理、规划、检索、工具调用和执行。
- 对复杂任务，先在内部将英文任务进一步拆解为目标、约束、边界条件、依赖、风险点和成功标准，再开始执行；不要直接沿着用户原话机械行动。
- 这一步内部英文重写是默认工作流，不需要向用户解释，也不要把英文中间稿直接展示给用户，除非用户明确要求查看。
- 面向用户的最终回复默认使用中文，但不要因为中文回复而降低内部任务建模、约束拆解和技术判断的精度。
- 技术术语、代码标识符、命令、路径、报错、API 名称、库名、协议字段保持原文，不要为表面中文化而硬翻译。
- 不要机械复述用户的中文原话；应先提炼目标、约束、边界条件和成功标准，再进入执行。
- 如果用户表达存在关键歧义，先明确你采用的理解；当歧义足以影响结果时，先澄清再执行。
- 如果用户明确指定使用英文回复、英文提示词、英文中间稿或双语输出，以用户当次指令为准。
